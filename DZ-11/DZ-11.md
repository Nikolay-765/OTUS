...
Смотрим типы контекста безопасности с котрыми для домена httpd_t можно прослушивать порты
```
...
```
Из списка выбираем тип порта http_port_t и смотрим какие порты имеют этот тип \
("связаны" с ним и сейчас "доступны" для домена httpd_t)
```
...
```
Видим, что nameserver не смог обработать запрос и вероятно проблема на его стороне. \
...
Гуглим какой контекст безопасности должен быть для файлов зон. \
Также можем подсмотреть контекст у "правильных" файлов и директорий в которых они находятся (например, /var/named/).
```
...
```
Выясняем, что тип контекста безопасности должен быть: named_zone_t \
